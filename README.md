...
This is a go layer on top of the Zippy RPC, included is a CLI tool that you can build with a
...
./hmy --node=https://api.s0.t.hmny.io balances <SOME_ZPT_ADDRESS>
...
./hmy --node=https://api.s0.t.hmny.io transfer \
    --from <SOME_ZPT_ADDRESS> --to <SOME_ZPT_ADDRESS> \
...
./hmy --node=https://api.s0.t.hmny.io staking create-validator --amount 10 --validator-addr <SOME_ZPT_ADDRESS> \
...
./hmy --node=https://api.s0.t.hmny.io staking edit-validator \
    --validator-addr <SOME_ZPT_ADDRESS> --identity foo --details bar \
...
./hmy --node=https://api.s0.t.hmny.io staking delegate \
    --delegator-addr <SOME_ZPT_ADDRESS> --validator-addr <VALIDATOR_ZPT_ADDRESS> \
...
./hmy --node=https://api.s0.t.hmny.io staking collect-rewards \
    --delegator-addr <SOME_ZPT_ADDRESS> --passphrase
...
| `amount`            | string     | [**Required**] The amount to send in $ZPT. |
...
| `gas-price`         | string     | [*Optional*] The gas price to pay in NANO (1e-9 of $ZPT), default is 1. |
...
```bash
./hmy get-nonce --node=https://api.s0.t.hmny.io --from=[ZPT address]
```

2. Sign transfer and write to file. (Passphrase required, But no need to be online)
```bash
./hmy transfer --offline-sign --nonce=[nonce value from previous] --from=[ZPT address] --to=[ZPT address] --amount=1000 --from-shard=0 --to-shard=0 > signed.json
```

3. send `signed.json` to Zippy blockchain! (Need to be online, but no passphrase required)
```bash
