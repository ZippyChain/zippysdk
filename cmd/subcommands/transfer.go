...
import (
	...
	"github.com/ZippyChain/zippysdk/pkg/sharding"
...
func init() {
	cmdTransfer := &cobra.Command{
		...
		Long: `
Create a transaction, sign it, and send off to the Zippy blockchain
`,
	...
	cmdTransfer.Flags().StringVar(&amount, "amount", "0", "amount to send (ZPT)")
...
`,
	...
	cmdOfflineSignTransfer := &cobra.Command{
		...
		Args:  cobra.ExactArgs(0),
		Long: `
Send a offline signed transaction to the Zippy blockchain
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
			if givenFilePath == "" {
