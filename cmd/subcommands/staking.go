...
import (
	...
	bls_core "github.com/ZippyChain/zippybls/ffi/go/bls"
	...
	"github.com/ZippyChain/zippysdk/pkg/ledger"
	"github.com/ZippyChain/zippysdk/pkg/rpc"
	"github.com/ZippyChain/zippysdk/pkg/store"
	"github.com/ZippyChain/zippysdk/pkg/transaction"
...
var (
	...
	errMinSelfDelegationTooSmall       = errors.New("min-self-delegation can not be less than 1 ZPT")
	errMaxTotalDelegationTooSmall      = errors.New("max-total-delegation can not be less than 1 ZPT")
...
func delegationAmountSanityCheck(minSelfDelegation *numeric.Dec, maxTotalDelegation *numeric.Dec, amount *numeric.Dec) error {
	// MinSelfDelegation must be >= 1 ZPT
...
func init() {
	cmdStaking := &cobra.Command{
		...
		Long: `
Create a staking transaction, sign it, and send off to the Zippy blockchain
`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.Help()
			return nil
