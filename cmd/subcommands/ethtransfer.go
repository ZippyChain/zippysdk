...
import (
	...
	rpcEth "github.com/ZippyChain/zippysdk/pkg/rpc/eth"
...
func ethHandlerForBulkTransactions(txLog *transactionLog, index int) error {
	...
	if txnFlags.GasPrice != nil {
		...
		gasPrice = "100" // Reset to default for subsequent transactions
	}
...
func init() {
	cmdEthTransfer := &cobra.Command{
		...
		Long: `
Create an Ethereum compatible transaction, sign it, and send off to the Zippy blockchain
`,
	...
	cmdEthTransfer.Flags().StringVar(&amount, "amount", "0", "amount to send (ZPT)")
	...
	cmdOfflineSignEthTransfer := &cobra.Command{
		...
		Args:  cobra.ExactArgs(0),
		Long: `
Send a offline signed transaction to the Zippy blockchain (on the same shard)
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
			if givenFilePath == "" {
